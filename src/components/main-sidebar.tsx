...
import { Folder, LogOut, Settings, User, Brain } from "lucide-react";
...
export function MainSidebar() {
  ...
  return (
    <div className="flex h-full w-60 p-5 bg-background">
            ...
            return (
              <Link key={item.name} href={item.href}>
                <Button
                  variant="ghost"
                  className={cn(
                    "w-full justify-start rounded-full gap-3",
                    currentTab === item.href && "bg-primary text-white"
                  )}
                >
                  {item.icon}
                  {item.name}
