...
export function InputForm({ currentStep, setCurrentStep }: MaterialInputProps) {
  ...
  const handleSaveAnswers = async () => {
    try {
      ...
      const projectId = projectData.selectedItem.id;
      const currentProjectThemeId =
        projectData.selectedItem.design_theme || null;
      ...
      for (const answerItem of answerList) {
        ...
        try {
          if (question.type === "number") {
            ...
            const isWiredSmoke =
              questionId === "67807c5d-0a41-4161-8d30-8f8f1ff28059";
            const isBatterySmoke =
              questionId === "7b55934a-d833-4f07-97d4-da14d32663a2";
            ...
            if ((isWiredSmoke || isBatterySmoke) && quantity > 0) {
                ...
                if (match) {
                  ...
                  if (productId && isThemeCompatible) {
                    productQuantities[productId] =
                      (productQuantities[productId] || 0) + count;
                    referencedProductIds.add(productId);
                    console.log(
                      `🧾 Added ${count} of ${size}-pack: ${productId}`
                    );
                  }
                }
              });
              ...
              productObjs.forEach((p: any) => {
                ...
                const isThemeCompatible =
                  !currentProjectThemeId ||
                  !themePath ||
                  themePath === currentProjectThemeId;
      ...
      const { data: fullExistingProducts, error: productThemeError } =
        await supabase
          .from("products")
          .select("id, themes")
      ...
      const productsToRemove = fullExistingProducts.filter((prod: any) => {
        ...
        if (!Array.isArray(productThemes) || productThemes.length === 0)
          return false; // keep unthemed
      ...
      for (const [productId, quantity] of Object.entries(productQuantities)) {
        if (quantity > 0) {
          ...
          if (deleteZeroError)
            console.error(
              "Failed to remove zero-qty product:",
              deleteZeroError
            );
        }
      }
      ...
      const toDelete = existingProductIds.filter(
        (id) => !referencedProductIds.has(id)
      );
      ...
      if (toDelete.length > 0) {
        ...
        if (cleanupError)
          console.error("Error removing deselected products:", cleanupError);
      }
              ...
              onSuggestion={(data) => {
                Object.entries(data).forEach(([key, value]) => {
                  ...
                  if (questionId) {
                    ...
                    console.log(
                      `✅ Updated ${questionId} with value: ${value}`
                    );
                  } else {
                    console.warn(
                      `❌ No question ID found for assistant key: ${key}`
                    );
                  }
                });
              }}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {currentQuestions.map((question: any, index: number) => {
                const answer =
                  answerList.find((a: any) => a.questionId === question.id)
                    ?.answer || "";
                return (
              ...
              {currentHeader.name === "Electrical" && (
                <div className="flex items-center space-x-4">
                  <p className="w-1/2 text-sm">
                    Where will semi-flush lights be installed?
                  </p>
                  <Input
                    type="text"
                    value={semiFlushText}
          ...
          <div className="flex justify-between pt-6">
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="bg-[#2365C8] text-white hover:bg-blue-700"
              >
                {currentCategoryIndex === headerData.list.length - 1
                  ? "Finish"
                  : "Next Category"}
              </Button>
            </div>
